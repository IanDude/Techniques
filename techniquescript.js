...
async function runCaesarAnimationDecryptASCII({ scene, camera, engine }, shift, stopLoopRef, message = "HELLO") {
    // If message is empty, don't animate
    if (!message || message.trim() === "") {
        return;
    }
    ...
    while (!stopLoopRef.value) {
        ...
        for (let i = 0; i < letterBoxes.length; i++) {
            ...
            // Caesar shift decryption (reverse of encryption)
            const originalIndex = (charIndex - shift + asciiCodes.length) % asciiCodes.length;
            const originalChar = asciiCodes[originalIndex];
            ...
            // Animate the reverse shift (decryption)
            let steps = shift;
            ...
            for (let s = 0; s <= steps; s++) {
                ...
                // Highlight current position (moving backwards)
                const targetIndex = (charIndex - s + asciiCodes.length) % asciiCodes.length;
                letterBlocks[targetIndex].color = (s === steps) ? "#03ff13" : "#ff0d05";
                letterBlocks[targetIndex].fontWeight = "bold";
                letterBlocks[targetIndex].background = (s === steps) ? "#22c55e" : "#3b82f6";
            ...
            // Update the box texture with the decrypted ASCII code
            letterBox.texture.clear();
            letterBox.texture.drawText(originalChar, null, null, "bold 48px Arial", "#000000", "#e0e7ef", true);
            letterBox.char = originalChar;
            letterBox.asciiIndex = originalIndex;
