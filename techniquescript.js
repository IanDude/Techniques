...
function runMain() {
  ...
  cards.forEach(card => {
    ...
    if(h2 && h2.textContent.trim().toLowerCase() === 'caesar') {
        ...
        // Hide the default modal canvas
        // Remove any previous caesar elements
        ...
        // Hide modal canvas for transposition
        ...
        try {
            ...
            async function displayBoxes({ scene, camera, engine }, message, key, stopLoopRef, animationId) {
                if (!message || !key) {
                    scene.meshes.slice().forEach(mesh => mesh.dispose());
                    if (transGuiTexture) {
            ...
            transPlaintextInput.addEventListener('input', async (e) => {
            ...
            transKeyInput.addEventListener('input', async (e) => {
